...
manifest (every file < 500 MB), produced by `scripts/db_delta_export.py` and applied
...
(`alembic current` on both; this bundle was produced at the 2.0.1 schema, no new
migrations since the 2026-08-03 export). Deploy the matching app image first or after —
the data load is independent of the image.

```bash
# 1. copy the directory into the app container (and the importer itself for images <= 2.0.1,
#    which predate it — from 2.0.2 the script ships in the image)
docker cp db_delta_YYYYMMDD sp_prod_app:/tmp/
docker cp scripts/db_delta_import.py sp_prod_app:/app/scripts/db_delta_import.py
